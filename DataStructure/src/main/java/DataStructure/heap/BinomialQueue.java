...
public final class BinomialQueue<AnyType extends Comparable<? super AnyType>>
{
    ...
    private static final int DEFAULT_TREES = 1; //默认二项树的个数

    private int currentSize;                // 数组内的元素
    private BinNode<AnyType> [ ] theTrees;  //存放二项树的数组
    ...
    public void merge( BinomialQueue<AnyType> rhs )
    {
        ...
        {
            BinNode<AnyType> t1 = theTrees[ i ];                                     //this二项树  代表被合并的二项树
            BinNode<AnyType> t2 = i < rhs.theTrees.length ? rhs.theTrees[ i ] : null;//rhs二项树     代表需要合并的二项数
